...
public class ManageDeliveryBoyPage extends BaseClass {
 ...
 DeliveryBoyPage dp;
	 ...
	 @Test public void verifyingResetButtonColor () { lp=new LoginPage(driver);
	 lp.presteps(); dp= new DeliveryBoyPage(driver); String
	 actual=dp.rebuttonColorCheck(); System.out.println(actual); String
	 expected="rgba(255, 193, 7, 1)"; Assert.assertEquals(actual,
	 expected,"color is not matching");
...
public void passwordNameValidation() { lp=new LoginPage(driver);
 ...
 Assert.assertEquals(actual,expected,Constant.nameerrormsg);
  ...
  @Test(groups= {"SmokeTest"})
  public void searchnameVallidation() { lp=new LoginPage(driver);
  lp.presteps(); dp= new DeliveryBoyPage(driver); String
  actual=dp.nameVallidation(); System.out.println(actual); String
  expected="Megha";
  //Assert.assertEquals(actual,expected,"Name is not matching");
  Assert.assertEquals(actual,expected,Constant.nameerrormsg);
  }
